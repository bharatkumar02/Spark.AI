...
export default function UserProfil({ isCollapsed }: Props) {
  return (
    <div className="p-4 border-t dark:border-gray-800 bottom-0 w-full relative">
      <button
        className={`flex items-center gap-3 w-full hover:bg-gray-800 p-2 rounded-lg transition-colors cursor-pointer peer
...
export function UserDetail({ isCollapsed }: Props) {
  return (
    <div
      className={`absolute shadow-md  shrink-0 whitespace-nowrap  px-2 rounded-lg bg-white invisible peer-focus:visible translate-y-2 peer-focus:translate-y-0 transition-all duration-300 opacity-0 peer-focus:opacity-100 dark:bg-gray-800 *:px-1.5 ${
        isCollapsed
          ? 'w-[17rem] m-3 left-full bottom-0 bg-red-600'
          : 'w-[95%] bottom-full left-1/2 -translate-x-1/2'
      }`}
    >
      <div className="text-center text-[15px] py-3.5">
      ...
      <ul className="border-y-[0.5px] *:py-2 py-2 *:flex *:items-center *:gap-3.5 text-[15px] *:cursor-pointer dark:border-gray-700">
        <li>
